...
setwd("/Users/karismashaik/Desktop/capstone/data")

##  Reading H1B Visa Petition data from 2011-2016

h1b_data<-read.csv('h1b16.csv')
print(head(h1b_data))
...
## Continious Distribution Function (CDF) 
...
## Sample of wages and filter out NA values
set.seed(123)
...
set.seed(1)
sample <- sample(c(TRUE, FALSE), nrow(data_science_df), replace=TRUE, prob=c(0.8,0.2))
train.data  <- data_science_df[sample, ]
test.data   <- data_science_df[!sample, ]
